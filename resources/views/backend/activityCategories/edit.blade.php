<x-backend.layouts.master>
    ...
    <div class="overflow-x-auto">
        ...
        <form action="{{ route('activity-categories.update', $activityCategory->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="title" class="block text-sm font-medium text-gray-700">Title</label>
                <input type="text" name="title" id="title" value="{{ old('title', $activityCategory->title) }}" required
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
            </div>

            <div class="mb-4">
                <label for="alias" class="block text-sm font-medium text-gray-700">Alias</label>
                <input type="text" name="alias" id="alias" value="{{ old('alias', $activityCategory->alias) }}" readonly
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
            </div>
            ...
            <div class="mb-4 flex items-center gap-2">
                ...
                <span id="statusText" class="text-xs font-medium text-gray-700">
                    {{ $activityCategory->status === 'active' ? 'Active' : 'Inactive' }}
                </span>

                <!-- Hidden input for form -->
                <input type="hidden" name="status" id="status"
                    value="{{ $activityCategory->status === 'active' ? '1' : '0' }}">
            </div>
