    ...
    <x-slot name="header">
        <div class="flex items-center justify-between px-4 py-4 border-b lg:py-6 dark:border-primary-darker">
            <h2 class="text-2xl font-semibold">
                {{ __('Edit Inclusion: ') }} {{ $inclusion->title }}
            </h2>
        </div>
    </x-slot>
    ...
    <div class="overflow-x-auto">
        ...
        <form action="{{ route('inclusions.update', $inclusion->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $inclusion->title) }}" required
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
            </div>

            <div class="mb-4">
                <label for="country_code" class="block text-sm font-medium text-gray-700">Inclusion Code</label>
                <input type="text" name="country_code" id="country_code" value="{{ old('country_code', $inclusion->country_code) }}"
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
            </div>
            ...
            <div class="mb-4 flex items-center gap-2">
                ...
                <input type="hidden" name="status" id="status"
                    value="{{ $inclusion->status === 'active' ? '1' : '0' }}">
            </div>
