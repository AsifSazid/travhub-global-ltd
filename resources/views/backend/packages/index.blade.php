    ...
    <div class="overflow-x-auto">
        ...
        <div class="mt-4 px-6">
            ...
            <div class="px-4 py-2 bg-gray-100 border-t text-sm text-gray-500 flex justify-between items-center">
                <a href="{{ route('backend.packages.trash') }}" class="text-red-500 hover:text-red-700 mx-1"
                    title="Trash Lists">
        ...
        <script>
            document.addEventListener("DOMContentLoaded", () => {
                ...
                const fetchRoles = async (search = '', page = 1) => {
                    try {
                        const response = await fetch(
                            `{{ route('backend.packages.getData') }}?search=${search}&page=${page}`, {
                                        ...
                                        <div class="flex justify-center">
                                            <a href="/admin/packages/${package.uuid}/show" class="px-1 text-blue-500 hover:text-blue-700" title="View"><i class="fas fa-eye"></i></a>
                                                ${(package.status === 'inactive' && package.completion_status === 'incomplete')
                                                    ? `<a href="/admin/packages/${package.uuid}/step/${package.progress_step ? package.progress_step : 1}" class="px-1 text-yellow-500 hover:text-yellow-700" title="Edit">
                ...
                document.getElementById("downloadPdfBtn").addEventListener("click", () => {
                    ...
                    let url = `{{ route('backend.packages.download.pdf') }}`;
                    if (search) {
