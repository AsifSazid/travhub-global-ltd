<div id="itinerary-section" class="space-y-6">
    ...
    <!-- Dynamic Day Cards -->
    <div id="day-container" class="space-y-6"></div>
    ...
    <!-- Live JSON preview (optional, can remove later) -->
...
@push('js')
    <script>
        /* ---------------------------
          Data & Preset Configuration
           - itinerary: main array (user requested)
        ----------------------------*/
        let itinerary = []; // stores all day objects
  ...
  `;
            ...
            wrapper.querySelector('.delete-day-btn').addEventListener('click', () => {
                ...
                itinerary = itinerary.filter(d => d.id !== day.id);
                // reassign day numbers
                itinerary.forEach((d, i) => d.dayNumber = i + 1);
                renderAll();
            });
  ...
  `;
        ...
        addSelectedPresetsBtn.addEventListener('click', () => {
            ...
            checked.forEach(cb => {
                ...
                if (presetActivities[idx]) {
                    ...
                    act._uid = 'a' + (Date.now() + Math.floor(Math.random() * 1000)) + Math.floor(Math
                        .random() * 100);
    ...
    `;
        ...
        addDayBtn.addEventListener('click', () => {
            const newDay = createEmptyDay(itinerary.length + 1);
