    ...
    <div class="overflow-x-auto">
        ...
        <form action="{{ route('currencies.update', $currency->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $currency->title) }}"
                    required
            ...
            <div class="mb-4">
                ...
                <input type="text" name="icon" id="icon" value="{{ old('icon', $currency->icon) }}"
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
            </div>
                ...
                <select name="country_uuid" id="country_uuid"
                    ...
                    @foreach ($countries as $country)
                        <option value="{{ $country->uuid }}"
                            {{ $currency->country_uuid == $country->uuid ? 'selected' : '' }}>
                            {{ $country->title }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4 flex items-center gap-2">
                ...
                <input type="hidden" name="status" id="status"
                    value="{{ $currency->status === 'active' ? '1' : '0' }}">
            </div>
