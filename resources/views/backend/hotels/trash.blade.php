<x-backend.layouts.master>
    ...
    <div class="overflow-x-auto">
        <table id="roleTable" class="w-full table-striped table-bordered text-sm">
            <thead class="bg-gray-100 text-gray-700 uppercase">
                <tr>
                    ...
                    <th class="px-6 py-4">Hotel Code</th>
            ...
            <tbody class="text-center">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex justify-center space-x-2">
                                <form action="{{ route('hotels.restore', $trashed->uuid) }}" method="POST"
                                    class="d-inline">
                                    @csrf
                                ...
                                <form action="{{ route('hotels.forceDelete', $trashed->uuid) }}" method="POST"
                                    class="inline ml-2"
                                    onsubmit="return confirm('Permanently delete this city?');">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="7" class="py-4 text-center text-gray-500">No city found!
                        </td>
                    </tr>
                @endforelse
            </tbody>
        ...
        <div class="mt-4 px-4 py-2 bg-gray-100 border-t text-sm text-gray-500 flex justify-between items-center">
            <a href="{{ route('hotels.index') }}" class="text-blue-500 hover:text-blue-700 mx-1"
                title="Hotel Lists"><i class="fas fa-trash-alt"></i> Hotel Lists</a>
        </div>
    </div>
