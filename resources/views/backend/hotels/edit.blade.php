    ...
    <div class="overflow-x-auto">
        ...
        <form action="{{ route('hotels.update', $hotel->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" id="title" required value="{{ old('title', $hotel->title) }}"
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
            </div>

            <div class="mb-4">
                <label for="city_uuid" class="block text-sm font-medium text-gray-700">Country</label>
                <select name="city_uuid" id="city_uuid"
                    ...
                    @foreach ($cities as $city)
                        <option value="{{ $city->uuid }}"
                            {{ $hotel->city_uuid == $city->uuid ? 'selected' : '' }}>
                            {{ $city->title }}
                        </option>
                    @endforeach
                </select>
