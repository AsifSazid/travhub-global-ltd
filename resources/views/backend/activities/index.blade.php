<x-backend.layouts.master>
    <x-slot name="header">
        <div class="flex items-center justify-between px-4 py-4 border-b lg:py-6 dark:border-primary-darker">
            ...
            <a href="{{ route('activities.create') }}" class="text-green-500 hover:text-green-700 mx-1" title="Create">
    ...
    <div class="overflow-x-auto">
        ...
        <div class="mt-4 px-6">
            ...
            <div class="px-4 py-2 bg-gray-100 border-t text-sm text-gray-500 flex justify-between items-center">
                <a href="{{ route('activities.trash') }}" class="text-red-500 hover:text-red-700 mx-1"
                    title="Trash Lists">
            ...
            document.addEventListener("DOMContentLoaded", () => {
                ...
                const fetchRoles = async (search = '', page = 1) => {
                    try {
                        const response = await fetch(
                            `{{ route('activities.getData') }}?search=${search}&page=${page}`, {
                ...
                const renderTable = (activities) => {
                    ...
                    if (!activities || activities.length === 0) {
                        tableBody.innerHTML =
                            `<tr><td colspan="5" class="py-4 text-center text-gray-500">No activities found</td></tr>`;
                        return;
                    }
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex justify-center">
                                            ...
                                            <a href="/activities/${activity.uuid}/edit" class="px-1 text-yellow-500 hover:text-yellow-700" title="Edit"><i class="fas fa-edit"></i></a>
                                            <form action="/activities/${activity.uuid}" method="POST" onsubmit="return confirm('Move to trash?')">
                ...
                document.getElementById("downloadPdfBtn").addEventListener("click", () => {
                    ...
                    let url = `{{ route('activities.download.pdf') }}`;
                    if (search) {
