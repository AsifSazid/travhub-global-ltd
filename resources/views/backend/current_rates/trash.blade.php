<x-backend.layouts.master>
    ...
    <div class="overflow-x-auto">
        <table id="roleTable" class="w-full table-striped table-bordered text-sm">
            <thead class="bg-gray-100 text-gray-700 uppercase">
                <tr>
                    ...
                    <th class="px-6 py-4">Currency Rate Name</th>
                    <th class="px-6 py-4">Currency Rate Code</th>
            ...
            <tbody class="text-center">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex justify-center space-x-2">
                                <form action="{{ route('current_rates.restore', $trashed->uuid) }}" method="POST"
                                    class="d-inline">
                                    @csrf
                                ...
                                <form action="{{ route('current_rates.forceDelete', $trashed->uuid) }}" method="POST"
                                    class="inline ml-2"
                                    onsubmit="return confirm('Permanently delete this current_rate?');">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="7" class="py-4 text-center text-gray-500">No current_rate found!
                        </td>
                    </tr>
                @endforelse
            </tbody>
        ...
        <div class="mt-4 px-4 py-2 bg-gray-100 border-t text-sm text-gray-500 flex justify-between items-center">
            <a href="{{ route('current_rates.index') }}" class="text-blue-500 hover:text-blue-700 mx-1"
                title="Currency Rate Lists"><i class="fas fa-trash-alt"></i> Currency Rate Lists</a>
        </div>
    </div>
